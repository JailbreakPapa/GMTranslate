...
//
// This is the same as the following code, but with knowledge/assumptions of the expected zeros and ones in GetScreenToCameraMatrix() (DX scheme)
...
float3 FastScreenCoordToViewSpace(float2 normalizedScreenCor, float depthFromZBuffer)
{
  ...
  float linearZ = LineariznsBufferDepth(depthFromZBuffer);
...
{
  return float3(float2(GetScreenToCameraMatrix()._11, GetScreenToCameraMatrix()._22) * normalizedScreenCor * linearDepth, linearDepth);
}
